...
//         if(!token) {
//             throw new Error("JWT token을 찾을 수 없습니다.");
//         }
        
//         const response = await fetch(`${BASE_URL}/api/v1/gamelogic/${gameRoomId}/trade/investments`, {
//             method: 'GET',
//             headers: {
...
//                     `/topic/gameRoom/${gameRoomId}/investments`,
//                     (message) => {
//                         try {
...
export default function MyInvest() {
    const { investments, balance, valuationAmount, returnRate, netAssets, fetchInvestments } =
        useInvestments();
    const { stompClient } = useWebSocket();
    const pathname = usePathname();
    const gameRoomId = pathname.split("/")[2];
    ...
    useEffect(() => {
        if (gameRoomId) {
        fetchInvestments(gameRoomId);
        }
    }, [gameRoomId, fetchInvestments]);
    ...
    useEffect(() => {
        ...
        (message) => {
            try {
            const updatedInvestments = JSON.parse(message.body);
            console.log("Investment update received:", updatedInvestments);

            // 실시간으로 fetchInvestments 호출하여 상태 업데이트
            fetchInvestments(gameRoomId);
            } catch (error) {
            console.error("Error processing investment update:", error);
            }
    ...
    }, [stompClient, gameRoomId, fetchInvestments]);
    ...
    return (
        ...
        <div className="content">
            ...
            <p className="text-amount-large">{valuationAmount.toLocaleString()}원</p>
            <p
                className={`text-profit-large ${
                returnRate >= 0
                    ? "text-profit-positive-large mt-1"
                    : "text-profit-negative-large mt-1"
                }`}
            >
                수익률: {returnRate.toFixed(2)}%
            </p>
            </div>
            ...
            {investments.length > 0 ? (
                investments.map((investment) => (
                <div key={investment.ticker} className="investment-item">
                    <div className="investment-image">
                    <Image
                        src={`/images/${investment.companyName}.png`}
                        alt={investment.ticker}
            ...
            <MyBalance balance={balance} balanceHistory={[]} />
        </div>
        </div>
    );
}
